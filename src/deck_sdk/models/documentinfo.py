"""Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT."""

from __future__ import annotations
from datetime import datetime
from deck_sdk.types import BaseModel, Nullable, OptionalNullable, UNSET, UNSET_SENTINEL
from pydantic import model_serializer
from typing import Dict, Optional
from typing_extensions import NotRequired, TypedDict


class DocumentInfoTypedDict(TypedDict):
    document_id: str
    file_type: str
    metadata: NotRequired[Dict[str, str]]
    file_name: NotRequired[Nullable[str]]
    file_size: NotRequired[Nullable[int]]
    fetch_date: NotRequired[datetime]


class DocumentInfo(BaseModel):
    document_id: str

    file_type: str

    metadata: Optional[Dict[str, str]] = None

    file_name: OptionalNullable[str] = UNSET

    file_size: OptionalNullable[int] = UNSET

    fetch_date: Optional[datetime] = None

    @model_serializer(mode="wrap")
    def serialize_model(self, handler):
        optional_fields = ["metadata", "file_name", "file_size", "fetch_date"]
        nullable_fields = ["file_name", "file_size"]
        null_default_fields = []

        serialized = handler(self)

        m = {}

        for n, f in type(self).model_fields.items():
            k = f.alias or n
            val = serialized.get(k)
            serialized.pop(k, None)

            optional_nullable = k in optional_fields and k in nullable_fields
            is_set = (
                self.__pydantic_fields_set__.intersection({n})
                or k in null_default_fields
            )  # pylint: disable=no-member

            if val is not None and val != UNSET_SENTINEL:
                m[k] = val
            elif val != UNSET_SENTINEL and (
                not k in optional_fields or (optional_nullable and is_set)
            ):
                m[k] = val

        return m
